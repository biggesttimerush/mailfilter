...
if [ $1 == "dnf" ]; then
  ...
  sudo $pm_name install -q -y clamav clamav-update clamav-server-systemd
...
sudo sed -i "186 i ##### ClamAV virus scanning\n    if (message_contains_virus()) {\n        md_syslog('warning', 'Virus detected in email. Message dropped.');\n        return action_bounce('Email contains a virus and has been rejected.');\n    }" /etc/mail/mimedefang-filter
...
# ClamAV helper function in /etc/mail/mimedefang-filter
# Add the following manually or via your provisioning logic in the file:

sub message_contains_virus {
    my($code, $category, $message) = message_contains_virus_clamd();
    return $code eq "VIRUS";
}
